...
- Balena's displayed release version comes from `balena.yml`'s `version`; the release workflow commits the numeric Git tag version to `main` before `balena push` and moves the tag to that metadata commit.
